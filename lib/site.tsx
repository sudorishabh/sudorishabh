...
/*
  Single source for the profile links. They surface in three places -- the nav
  pill, the hero, and the footer -- and were previously copy-pasted into each,
  so an edit to one silently left the other two wrong.

  The brand marks stay hand-written: lucide dropped its brand icon set in v1,
  and these two are the only ones the site needs.
*/

export type SocialLink = {
  label: string;
  href: string;
  /** Shown in the footer chip; the nav uses the icon alone. */
  handle: string;
  icon: ReactNode;
};
...
export const socialLinks: SocialLink[] = [
  ...
  },
  {
    ...
    handle: "rishabhnegi175@gmail.com",
    icon: (
      <svg
        viewBox='0 0 24 24'
        fill='none'
